                   ...
                   const float* const filter, const int filterWidth)
{
  ...
  for(int r = 0; r < filterWidth; r++) {
    for(int c = 0; c < filterWidth; c++) {
      int cur_x = abs_x + c - filterWidth/2;
      int cur_y = abs_y + r - filterWidth/2;
      cur_x = min(max(cur_x, 0), numCols-1);
      cur_y = min(max(cur_y, 0), numRows-1);
      result += filter[c*filterWidth + r] 
                * static_cast<float>(inputChannel[cur_x + cur_y*numCols]);
    }
  }
  outputChannel[abs_x + abs_y*numCols] = result;
